...
async fn self_id(ctx: &Ctx) -> ProbeResult {
    let p = ProbeResult::new("self_id", "自我身份声明", G).weight(2);
    ...
    match (claimed_family, observed) {
        (Some(c), Some((o, n))) if c == o => {
            p.pass(format!("自称 {o}，与宣称的模型家族一致（{n} 项标记）"))
        }
        (Some(c), Some((o, _))) => p
            .fail(format!("宣称 {c}，但自称 {o}"))
            .finding("表面自述可以被 system prompt 伪造，需与能力档位一起判读"),
        (Some(_), None) => p
            .warn("回答里没有可识别的家族标记")
            .finding("模型拒绝自述，或中间层屏蔽了身份问题"),
        (None, Some((o, _))) => p
            .warn(format!("自称 {o}，但无法从模型名推断应有的家族"))
            .neutral(),
        (None, None) => p.warn("无法判断家族").neutral(),
    }
}

async fn meta_creator(ctx: &Ctx) -> ProbeResult {
    let p = ProbeResult::new("meta_creator", "创造者自述", G)
        .weight(1)
        .neutral();
    let Some((text, took)) = ask(
        ctx,
    ...
    let p = p
        ...
        .took(took);
    match observed {
        Some((f, _)) => p.pass(format!("自述创造者归属：{f}")),
        None => p.warn("创造者自述无法归类"),
    }
}

async fn context_claim(ctx: &Ctx) -> ProbeResult {
    let p = ProbeResult::new("context_claim", "上下文长度自述", G)
        .weight(1)
        .neutral();
    let Some((text, took)) = ask(
        ctx,
    ...
    match n {
        ...
        Some(v) if v >= 1000.0 => p.pass(format!("自述上下文 {} token", v as u64)),
        Some(v) => p.warn(format!("自述上下文 {v} —— 数值不合常理")),
        None => p.warn("没有给出可解析的数字"),
    }
}
...
const DATED_FACTS: &[(&str, f64, &str, &[&str])] = &[
    (
        "英国首相",
        2024.5,
...
async fn cutoff_claim(ctx: &Ctx) -> ProbeResult {
    let p = ProbeResult::new("cutoff_claim", "训练截止自述", G)
        .weight(1)
        .neutral();
    let Some((text, took)) = ask(
        ctx,
    ...
    let p = p
        ...
        .took(took);
    match ym {
        Some(v) => p.pass(format!("自述训练截止 {}", fmt_year_month(v))),
        None => p.warn("没有给出可解析的截止日期"),
    }
}
...
async fn world_knowledge(ctx: &Ctx) -> ProbeResult {
    let p = ProbeResult::new("world_knowledge", "时事知识截止", G).weight(2);
    ...
    for (label, date, question, accepted) in DATED_FACTS {
    ...
    if asked == 0 {
        return p.error("所有时事探针都失败了").took(took);
    }
    ...
    match (claimed, known_through) {
        ...
        (Some(c), Some(k)) if c + 0.3 < k => p
            ...
            .finding("模型普遍低报自己的训练截止，这个方向不构成风险信号")
            .took(took),
        (Some(c), Some(k)) if k + 0.5 < c => p
            ...
            .finding("自述比实际知识新——背后的模型比它声称的旧")
            .took(took),
        (_, Some(k)) => p
            .pass(format!("知识覆盖到 {}，与自述一致", fmt_year_month(k)))
            .took(took),
        (Some(c), None) => p
            .fail(format!(
                "自述截止 {}，但三道已知时事题全部答错",
            ...
            .finding("实际知识明显早于自述")
            .took(took),
        (None, None) => p.warn("既没有自述截止也答不出时事题").took(took),
    }
}
...
async fn capability_battery(ctx: &Ctx) -> (ProbeResult, BatteryResult) {
    let p = ProbeResult::new("capability", "能力档位电池", G).weight(3);
    ...
    p = p.finding("题目每次运行现场生成，供应商无法缓存答案".to_string());
...
async fn verbosity(ctx: &Ctx) -> ProbeResult {
    let p = ProbeResult::new("verbosity", "默认冗长度", G)
        .weight(1)
        .neutral();
    ...
    p.pass(format!("默认回答 {chars} 字符（{band}）"))
        .metric("band", band)
}
...
fn tier_estimate(ctx: &Ctx, b: &BatteryResult) -> ProbeResult {
    ...
    if margin < MIN_MARGIN {
        return p
            ...
            .warn(format!(
                "档位候选过于接近（最高分与次高分仅差 {margin:.3}），不下判断"
            ))
            .finding("样本量不足以区分相邻档位，请用 --depth forensic 重跑");
    }
    if asked_total < MIN_QUESTIONS {
        return p
            .metric("tier_severity", 0)
            .warn(format!(
                "只做了 {asked_total} 道能力题，不足以判定档位（实测倾向{}）",
                estimated.label_zh()
            ))
            .finding(format!(
                "至少需要 {MIN_QUESTIONS} 道题才会给出档位结论，请用 --depth balanced 或 forensic 重跑"
            ));
    }
    ...
    match drop {
        ...
        d if d < 0 => p
            .pass(format!(
                "实测{}，高于宣称的{}",
                estimated.label_zh(),
                claimed_tier.label_zh()
            ))
            .finding("能力高于宣称档位不是欺诈，不计入风险；也可能是本轮题目偏简单"),
        1 => p
            .warn(format!(
                "宣称{}，实测{}",
                claimed_tier.label_zh(),
            ...
            .finding("相差一档，可能是降级，也可能是采样偏差，未认定为降智"),
        _ => p
            .fail(format!(
                "宣称{}，实测{}——低了 {severity} 档",
                claimed_tier.label_zh(),
            ...
            .finding("跨档位的能力差距无法用采样噪声解释，这是降智的直接证据"),
    }
}
